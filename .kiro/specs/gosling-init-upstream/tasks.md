...
- [x] 1. Add CLI flags and helper functions
  - [x] 1.1 Add `--remote-name`, `--remote-url`, and `--branch` flag variables and register them on `initCmd`
  ...
  - [x] 1.2 Implement `isTerminal()` helper function
    - Use `os.Stdin.Stat()` and check for `os.ModeCharDevice` to detect terminal stdin
    - Return `bool`
    - _Requirements: 3.5, 3.6_

  - [x] 1.3 Implement `promptWithDefault(prompt, defaultVal string) string` helper function
  ...
  - [x] 1.4 Implement `addGitRemote(dir, name, url string) error` helper function
  ...
  - [x] 1.5 Write property test for `promptWithDefault` — Property 2: Prompt default value preservation
...
- [x] 2. Implement Git repository initialization
  - [x] 2.1 Implement `initGitRepo(dir string) error` helper function
  ...
  - [x] 2.2 Integrate `initGitRepo` into `runInit` after file creation
  ...
  - [x] 2.3 Write property test for `initGitRepo` — Property 1: Git initialization creates a valid repository
...
- [x] 3. Implement upstream remote configuration
  - [x] 3.1 Implement `configureUpstreamRemote(dir string) error` orchestration function
  ...
  - [x] 3.2 Integrate `configureUpstreamRemote` into `runInit` after `initGitRepo`
    - Call `configureUpstreamRemote(absPath)` only when `initGitRepo` succeeds
    - _Requirements: 1.3, 2.1_

  - [x] 3.3 Write property test for `addGitRemote` — Property 3: Git remote registration round-trip
  ...
  - [x] 3.4 Write unit tests for upstream remote configuration
...
- [x] 5. Update .fly templates
  - [x] 5.1 Update `defaultUglyFoxConfig()` to return corrected .fly syntax
  ...
  - [x] 5.2 Update `defaultMotherGooseConfig()` to return corrected .fly syntax
  ...
  - [x] 5.3 Write smoke tests for UglyFox template
  ...
  - [x] 5.4 Write smoke tests for MotherGoose template
...
- [x] 6. Update success output and next steps messaging
  - [x] 6.1 Update `runInit` success output to reflect Git and remote setup
  ...
  - [x] 6.2 Write unit tests for success output
