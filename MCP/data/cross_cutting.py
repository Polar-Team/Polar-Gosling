...
ARCHITECTURE_OVERVIEW: dict[str, Any] = {
    ...
    "services": {
        ...
        "Rift": {
            "type": "Go binary (cmd/rift/)",
            "role": "Shared cache and remote Docker build context server",
            "responsibilities": [
                "Proxy Docker API requests from runners to the local Docker daemon",
                "Cache Docker image tarballs to S3 (sync every 5 minutes)",
                "Serve image cache to runners for faster pulls",
                "Report state to MotherGoose API",
                "Auto-shutdown after idle-timeout (default 10m) with anti-flap guard",
            ],
            "repo": "Polar-Gosling (cmd/rift/ + internal/rift/)",
            "note": "Optional component — runners work without it. Job runners cannot use Rift. VM lifecycle (create/destroy) is driven by MotherGoose via OpenTofu; Rift only handles the serve loop.",
        },
        "GoslingCLI": {
            "type": "Go binary (cmd/gosling/ + cmd/rift/)",
            "role": "Developer CLI, .fly file parser, and runner/Rift entrypoints",
            "responsibilities": [
            ...
            "repo": "Polar-Gosling",
            "packages": {
                "internal/cli": "Command implementations (init, add, validate, fmt, deploy, rollback, parse, status, runner, lockbox)",
                ...
                "internal/lockbox": "Cloud secret store management. SecretStore interface with YCLockboxStore (Yandex Cloud Lockbox) and AWSSecretsManagerStore (AWS Secrets Manager) implementations. Handles create, list, verify operations, input validation, and secret URI generation/parsing.",
